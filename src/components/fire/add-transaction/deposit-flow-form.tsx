...
import { colors, Input, DateInput, Select, CurrencyCombobox, IconArrow } from '@/components/fire/ui';
...
export function DepositFlowForm({
  ...
  // Check if creating new deposit asset or editing
  const isCreatingNewDeposit = newAsset.show === 'to';
  const showInterestSettings = isCreatingNewDeposit || isEditMode;

  return (
    <div className="space-y-4">
      {/* From Account - hidden in edit mode */}
      {!isEditMode && (
        <AssetFieldSelector
          label={labels.from}
      ...
      <div className="grid grid-cols-2 gap-2">
        <Input
          label={isEditMode ? "Balance" : "Amount"}
          type="number"
          placeholder="0.00"
          value={form.amount}
      ...
      {/* Edit mode: Editable deposit name */}
      {isEditMode && (
        <Input
          label="Deposit Name"
      ...
      {/* To Deposit Account - hidden in edit mode */}
      {!isEditMode && (
        <AssetFieldSelector
          label={labels.to}
      ...
      {/* Interest Rate Settings - show when creating new deposit or editing */}
      ...
      {/* Date & Note - only for create mode (transactions) */}
      {!isEditMode && (
        <div className="grid grid-cols-2 gap-2">
          <DateInput
            label="Date"
      ...
      <FormActions
        loading={loading}
        submitLabel={isEditMode ? 'Update Deposit' : labels.button}
        onCancel={onCancel}
