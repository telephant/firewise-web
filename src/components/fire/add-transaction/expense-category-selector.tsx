...
import { useExpenseCategories, mutateExpenseCategories } from '@/hooks/fire';
import { expenseCategoryApi } from '@/lib/fire/api';
import {
  colors,
  Button,
  Input,
...
export function ExpenseCategorySelector({ value, onChange }: ExpenseCategorySelectorProps) {
  // Fetch expense categories directly - only when this component is rendered
  const { categories, isLoading: loading } = useExpenseCategories();

  // Create expense category
  const createExpenseCategory = useCallback(async (name: string) => {
    const response = await expenseCategoryApi.create({ name });
  ...
  // Categories are fetched directly via SWR hook
