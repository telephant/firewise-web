...
export function EditFlowDialog({
  ...
  const balanceAdjustment = useMemo(() => {
    ...
    if (flowType === 'income' && flow.to_asset) {
    ...
    } else if (flowType === 'buy' || flowType === 'sell') {
      // Buy/Sell: involves asset transfers
      const fromAsset = flow.from_asset || flow.source_asset;
      const toAsset = flow.to_asset || flow.asset;
      if (fromAsset) {
        const assetCurrency = fromAsset.currency || 'USD';
        adjustments.push({
      ...
      if (toAsset) {
        const assetCurrency = toAsset.currency || 'USD';
        adjustments.push({
          asset: toAsset.name,
          delta: difference,
          assetCurrency,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Transactions are now immutable - show deprecation message
    toast.error('Editing transactions is no longer supported. Please delete and recreate if needed.');
    onOpenChange(false);
  };
